...
public class ScriptMain : UserComponent
{
    ...
    public override void Input0_ProcessInputRow(Input0Buffer Row)
    {
        ...
        //Set Webservice URL
        string wUrl = "https://codex.ntcloudpbx.ie/api/v2.0.0/queue/query_status?token=" + strToken;
        ...
        try
        {
            ...
            foreach (var metric in outPutMetrics.queues)
            {
                Output0Buffer.AddRow();
                Output0Buffer.number = metric.number;

                foreach (var innerMetric1 in metric.queuestatus)
                {
                    if (innerMetric1.callercount != "")
                    { 
                        foreach (var innerMetric2 in innerMetric1.members)
                        {
                            Output0Buffer.AddRow();
                            Output0Buffer.agent = innerMetric2.agent;
                            Output0Buffer.agentstatus = innerMetric2.agentstatus;
                            Output0Buffer.callercount = innerMetric1.callercount;
                            Output0Buffer.number = metric.number;
...
public class Queue
{
    ...
    public List<Queuestatus> queuestatus { get; set; }
}

public class Queuestatus
{
    public string callercount { get; set; }
